/* EasyMarket stable Telegram compatibility layer v20260917-9. */
(function(){
'use strict';
...
function install(){var modal=document.getElementById('productModal');if(modal&&modal.dataset.emGuardInstalled!=='1'){modal.dataset.emGuardInstalled='1';modal.addEventListener('click',function(e){if(e.target!==modal)return;e.preventDefault();e.stopPropagation();if(typeof window.closeProductModal==='function')window.closeProductModal();else{modal.classList.remove('open');document.body.style.overflow=''}},true)}}
/* Handle the modal CTA before any document-level Telegram/legacy click bridge. */
window.addEventListener('click',function(e){var btn=e.target&&e.target.closest&&e.target.closest('#productModal .product-modal-actions .product-buy-now');if(!btn)return;var code=btn.getAttribute('onclick')||'',m=code.match(/addToCartFromModal\(\s*(\d+)\s*,\s*(\d+(?:\.\d+)?)\s*\)/);if(!m)return;e.preventDefault();e.stopImmediatePropagation();if(typeof window.addToCartFromModal==='function')window.addToCartFromModal(Number(m[1]),Number(m[2]));else if(window.showToast)window.showToast('Корзина ещё загружается, попробуйте ещё раз')},true);
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
})();
(function(){var style=document.createElement('style');style.id='em-order-cta-label';style.textContent='.product-buy-now{font-size:13px!important}.product-modal-actions .product-buy-now{font-size:0!important}.product-modal-actions .product-buy-now::after{content:"⚡ Оформить заказ";font-size:13px;font-weight:700}';(document.head||document.documentElement).appendChild(style)})();