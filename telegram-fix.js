/* EasyMarket stable Telegram compatibility layer v20260917-11. */
(function(){
'use strict';
...
function handleModalCta(e){var btn=getBtn(e);if(!btn)return false;var now=Date.now();if(Number(btn.dataset.emHandledAt||0)>now-1000){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();return true}var code=btn.getAttribute('onclick')||'',m=code.match(/addToCartFromModal\(\s*(\d+)\s*,\s*(\d+(?:\.\d+)?)\s*\)/);if(!m)return false;btn.dataset.emHandledAt=String(now);e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();if(typeof window.addToCartFromModal==='function'){window.addToCartFromModal(Number(m[1]),Number(m[2]));}else if(window.showToast){window.showToast('Корзина ещё загружается, попробуйте ещё раз')}return true}
window.addEventListener('touchstart',handleModalCta,true);
window.addEventListener('pointerdown',handleModalCta,true);
window.addEventListener('touchend',handleModalCta,true);
window.addEventListener('pointerup',function(e){var btn=getBtn(e);if(btn&&Number(btn.dataset.emHandledAt||0)>Date.now()-1200){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}},true);
window.addEventListener('click',handleModalCta,true);
