...
if(window.MutationObserver){
 var observer=new MutationObserver(function(mutations){
   mutations.forEach(function(m){m.addedNodes&&Array.prototype.forEach.call(m.addedNodes,function(node){if(node&&node.nodeType===1)renameCardCheckoutButtons(node);});});
 });
 observer.observe(document.body,{childList:true,subtree:true});
}

/* SELLER: exactly the same quantity/price concept as the admin form. */
function sellerOptionsBox(){
 ...
 wrap.innerHTML='<label style="font-size:13px;color:#fff;font-weight:700;display:block;margin-bottom:7px;">Количество и цена</label><div id="sellerPriceOptionsRows"></div><button type="button" class="seller-action" id="sellerAddPriceOptionBtn" style="margin-top:8px;width:100%;">＋ Добавить вариант количества</button><input id="sellerPriceOptions" type="hidden" value=""><div style="font-size:11px;color:#858b99;margin-top:7px;line-height:1.4;">Укажи, сколько штук можно купить и общую цену для каждого варианта. Покупатель сможет выбрать вариант прямо в товаре.</div>';
...
initAllRestores();setTimeout(initAllRestores,100);setTimeout(initAllRestores,500);setTimeout(initAllRestores,1200);
})();
